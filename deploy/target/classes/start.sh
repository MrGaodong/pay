...
if [ -n "$str" ]; then
	...
	if [[ "$JAVA_VERSION" > "1.8" ]]; then
		JAVA_OPTS="${jvm8.params}"
	else
		JAVA_OPTS="${jvm7.params}"
	fi
else
...
DUBBO_OPTS="-Ddubbo.reference.check=false"
...
then
        ...
        $JAVA $JAVA_OPTS $JAVA_DEBUG_OPT $SERVICE_OPTS -classpath $CLASSPATH $DUBBO_OPTS $MAIN_CLASS 1>>$base/logs/service.log 2>&1 &
